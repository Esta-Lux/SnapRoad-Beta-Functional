...
- **Sharing your own location**: Foreground publishes use `MapScreen` / `Dashboard` timers. **Background**: `friendLiveShareBackgroundTask` (`expo-task-manager` + `expo-location`) posts to `POST /api/friends/location/update` on a throttled interval when sharing is on and policy allows (`syncFriendLiveShareBackgroundFromPolicy` from Dashboard; MapScreen mirrors the same policy). iOS needs **Always** location for updates while backgrounded; **When In Use** still gets foreground updates.
