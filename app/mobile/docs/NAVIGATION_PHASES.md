...
## Phase 1b — JS launch engine, SDK opt-in

**Launch default:** `EXPO_PUBLIC_NAV_LOGIC_SDK` defaults to **off** in [`navFeatureFlags.ts`](../src/navigation/navFeatureFlags.ts), and **`eas.json`** production env sets it to `"0"`. Active trips use the **JavaScript Directions + RNMapbox pipeline** for route progress, reroute, ETA, turn cards, and voice. Set **`EXPO_PUBLIC_NAV_LOGIC_SDK=1`** only for internal headless Mapbox Navigation SDK experiments.
...
Headless SDK path (opt-in when logic SDK is on):
