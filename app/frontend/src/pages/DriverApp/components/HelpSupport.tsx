        ...
        <div className="flex-1 overflow-auto">
            ...
            <div className="p-4">
              ...
              <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
                <button onClick={() => setSelectedCategory('')}
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium ${!selectedCategory ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                  All
                </button>
                {categories.map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)}
                    className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium ${selectedCategory === cat ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                    {cat}
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {filteredFaqs.map(faq => (
                  <div key={faq.id} className="bg-slate-800 rounded-xl overflow-hidden">
                    <button onClick={() => setExpandedFaq(expandedFaq === faq.id ? null : faq.id)}
                      ...
                      {expandedFaq === faq.id ? (
                        <ChevronUp className="text-slate-400 flex-shrink-0" size={16} />
                      ) : (
          ...
          {tab === 'contact' && (
            <div className="p-4 space-y-4">
              <p className="text-slate-400 text-sm">Can't find what you're looking for? Send us a message and we'll get back to you within 24 hours.</p>
              ...
              <div>
                ...
                <textarea placeholder="Describe your issue or question..." value={contactForm.message}
                  ...
                  rows={5}
                  className="w-full bg-slate-800 rounded-xl px-3 py-2 text-white text-sm outline-none resize-none" />
              </div>
              ...
              <div className="border-t border-slate-800 pt-4">
                <p className="text-slate-500 text-xs text-center mb-3">Or reach us directly</p>
                <div className="flex gap-2">
                  <button onClick={() => toast('Opening email client...')}
                    className="flex-1 bg-slate-800 text-white py-2 rounded-xl text-sm flex items-center justify-center gap-2">
                    <Mail size={14} /> Email
                  </button>
